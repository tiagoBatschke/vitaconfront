...
const EditPoiScreen = () => {
 ...
 useEffect(() => {
  ...
  function fetchPoi() {
    ...
    axios.get(`http://127.0.0.1:8000/api/pois/${id}`,  {
      withCredentials: true,
        headers: {
            'Authorization': `Bearer ${token}`
        }
    ...
    .then(response => {
        ...
        setNome(response.data.poi.nome)
        setEndereco(response.data.poi.endereco)
        setCategoriaId(response.data.poi.categoria_id)
    })
    .catch(error => {
        console.error('Erro:', error);
    }); 
  ...
  function updatePoi() {
    ...
    axios.put(`http://127.0.0.1:8000/api/pois/${id}`, {
        nome: nome,
        endereco: endereco, 
            ...
            <div className='flex flex-col items-center justify-evenly w-[100%]'>     
                    <div className='w-[90%] h-[10vh] flex  items-center '>
                        ...
                        <div className={`w-[80%] p-1 ml-[0%] h-[4vh] border border-black  bg-white  ${ campoFaltante === true && !categoriaId  ? styledInput : 'border-black'}`}>
                              ...
                              <select onChange={handleSelectChange} value={categoriaId} className={`w-[100%] h-[100%] flex text-slate-400 `}>
                        ...
                        <div className='w-[13%] ml-[8.5%]  h-[6vh]'>
                            <button className='w-[100%] border border-[#70AD47]' onClick={updatePoi}>Salvar</button>
                        </div>
                    </div>  
            </div>
          </div>
